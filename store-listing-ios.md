# App-Store-Connect-Eintrag (Entwurf) – VOLLGAS

Nur eine Textvorlage zum Reinkopieren in App Store Connect, kein Teil der App selbst.
Wird erst relevant, sobald das Apple Developer Program aktiv ist.

## App-Name (max. 30 Zeichen)
VOLLGAS

## Untertitel / Subtitle (max. 30 Zeichen)
Vollgas in die Prüfung

## Werbetext / Promotional Text (max. 170 Zeichen, jederzeit ohne Review änderbar)
Über 300 Fragen für deine Kfz-Mechatroniker-Abschlussprüfung. Gratis starten, Vollversion für alle Fragen, Battle Mode & Klausursimulation freischalten.

## Beschreibung (max. 4000 Zeichen)

VOLLGAS ist die Karteikarten-App für deine Kfz-Mechatroniker-Abschlussprüfung – von einem Azubi für Azubis gemacht.

🔧 Über 300 Fragen aus allen Prüfungsbereichen
Bremsanlage, Motor, Elektrik & Elektronik, Fahrwerk, Hochvolt, Klimaanlage, Kraftübertragung, Motormanagement & Abgasnachbehandlung, Bus-Systeme & Diagnose – alle Themen, die in der Abschlussprüfung drankommen.

📚 Ausführliche Erklärungen
Nicht nur auswendig lernen – jedes Themengebiet gibt's auch als verständliche Erklärung mit Diagrammen, damit du die Zusammenhänge wirklich verstehst.

🎓 Realistische Prüfungssimulation
40 zufällige Fragen, ausgewogen über alle Kategorien verteilt, mit Zeitlimit – genau wie in der echten Prüfung. Bestehensgrenze bei 75%.

⚔️ Battle Mode
Lade einen Freund per Link ein und vergleicht euren Lernfortschritt direkt in der App.

🔥 Streak & Ränge
Bleib jeden Tag am Ball und steig vom Azubi zum Obermeister auf.

✅ Merkt sich deinen Fortschritt
Falsch beantwortete Fragen kommen automatisch häufiger dran, bis sie sitzen. Dein Fortschritt ist an dein Konto gebunden und übersteht Neuinstallation und Gerätewechsel.

📴 Funktioniert offline
Einmal geladen, lernst du auch ohne Internetverbindung weiter.

💳 Gratis starten
10 Fragen pro Kategorie kostenlos. Mit der Vollversion (einmaliger Kauf, kein Abo) schaltest du alle Fragen, den Klausurmodus, Battle Mode, Wiederholungen und alle Erklärungen frei.

Kein Tracking. Keine Werbung. Einfach eine ehrliche Lern-App fürs Bestehen der Abschlussprüfung.

## Keywords (max. 100 Zeichen, kommagetrennt, nicht sichtbar für Nutzer)
kfz,mechatroniker,prüfung,ausbildung,azubi,karteikarten,lernen,kfz mechaniker,ihk,werkstatt,auto

## Kategorie
Primär: Bildung
Sekundär: Nachschlagewerke (optional)

## Support-URL
https://timkrgr.github.io/KFZ/privacy.html
(oder eine eigene Kontaktseite, falls gewünscht – Pflichtfeld, Apple verlinkt sie aus der App-Seite)

## Marketing-URL (optional)
https://timkrgr.github.io/KFZ/

## Datenschutzerklärung-URL
https://timkrgr.github.io/KFZ/privacy.html

## Copyright
© 2026 Tim Krüger

## Altersfreigabe (App Store Age Rating Questionnaire)
Keine Gewalt, kein nutzergenerierter Kontakt mit Fremden (Battle Mode läuft nur über
Einladungslinks zwischen bekannten Personen, kein offener Chat/Matching), unbeschränkte
Web-Zugriffe: nein. In-App-Käufe: ja (nicht-verbrauchbar, "Vollversion"). Erwartete
Einstufung: 4+.

## In-App-Kauf (im App-Store-Connect-Produktkatalog anzulegen)
- Referenzname: Vollversion
- Produkt-ID: com.timkrgr.oktan.vollversion
- Typ: Nicht-verbrauchbar (Non-Consumable)
- Preis: 7,99 € (entsprechende Preisstufe wählen)

## Screenshots (noch zu erstellen, pro Gerätegröße getrennt)
Pflicht: 6,7" (iPhone 15/16 Pro Max, 1290×2796px)
Optional zusätzlich: 6,5" (1284×2778 oder 1242×2688px)
Falls iPad-Unterstützung gewünscht: 12,9" iPad (2048×2732px) – sonst in App Store Connect
"nur iPhone" markieren, dann entfällt das.

Motive (wie Play Store, gleiche Screens wiederverwendbar):
- Home / Kategorien-Übersicht (inkl. Gratis-Hinweis "10 von X kostenlos")
- Prüfungssimulation
- Battle Mode
- Erklärungen
- Vollversion-Freischalt-Screen
- Streak-Feier-Animation

## App-Vorschau-Video (optional)
Nicht Pflicht, kann später ergänzt werden.
